...
def aula_realizada(request, aula_schema: AulaRealizadaSchema):
    ...
    return 200, f"Aula realizada com sucesso para o aluno(a) {aluno.nome}"
...
def update_aluno(request, aluno_id: int, aluno_data: AlunosSchema):
    ...
    if int(idade.days/365) < 18 and aluno_data.dict()['faixa'] in ('A', 'R', 'M', 'P'):
        raise HttpError(400, "Aluno menor de idade não podem receber esta faixa")

    print(aluno_data.dict().items())

    for attr, value in aluno_data.dict().items():
        if value:   #Caso possua valor, altera os dados
